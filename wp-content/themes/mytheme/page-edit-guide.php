...
// LOGGED IN AND IS AUTHOR
	if ($mypost->post_status == 'draft') {
		if ($_GET['ref'] == 'create') {	
			...
			echo '<div class="block-instruct"><p class="instructions">When you&#39;re ready to publish this CityHow Guide, click "Publish Guide." CityHow Editors will email you when it&#39;s been posted  so you can share the link with your friends!</p></div>';
			echo '<ul>';
			echo $btn_preview;
			...
			echo '<div id="edit-gde"'.do_shortcode('[formidable id=12]').'</div>';
		}
		elseif ($_GET['ref'] == 'update') {	
			// if error dont show update msg
			if (isset($my_form_error)) { }
			else {
				echo '<div class="alert alert-info"><a href="#" class="close" data-dismiss="alert">×</a><strong>Changes to this Guide were saved!</strong></div>';
			}
				echo '<div class="block-instruct"><p class="instructions">When you&#39;re ready to publish this CityHow Guide, click "Publish Guide." CityHow Editors will email you when it&#39;s been posted  so you can share the link with your friends!</p></div>';
			echo '<ul>';
			echo $btn_preview;
...
//			echo '<div style="clear:both;"></div>';
		...
		elseif (!isset($_GET['action']) AND !isset($_GET['ref'])) {
			echo '<div class="block-instruct"><p class="instructions">Looking for your CityHow Guides? Use the menu on the right to select an item to edit.</p></div>';
		}	
	}

	elseif ($mypost->post_status == 'pending') {
		if ($_GET['ref'] == 'review') {
			echo '<div class="alert alert-info"><a href="#" class="close" data-dismiss="alert">×</a><strong>This CityHow Guide was submitted for review!</strong></div>';
			echo '<p class="instructions">CityHow Editors will quickly review your Guide. Then they&#39;ll email you when it&#39;s posted so you can share the link with your friends</p>';
		...
		elseif (isset($_GET['action'])) {
			echo '<div class="block-instruct"><p class="instructions"><strong>This CityHow Guide is being reviewed.</strong></p>';
			echo '<p class="instructions">CityHow Editors will email you when it&#39;s posted so you can share the link with your friends</p></div>';
			echo '<div class="block-instruct"><p class="instructions">Click "Preview" to see what it will look like when it&#39;s published. If you want to work on another Guide, select it from the list on the right.</p>';
			echo '<p><a href="'.$app_url.'/?post_type=post&p='.$item_post_id.'&preview=true" title="See what it will look like" target="_blank"><button class="nh-btn-blue">Preview Guide</button></a></p></div>';
		}
		// if user went to entry w/o &action
...
//			echo '<div class="block-instruct"><p class="instructions"><strong>This <a href="'.get_permalink($item_post_id).'" title="View your CityHow Guide" target="_blank">CityHow Guide</a> has been published!</strong></p>';
			echo '<div class="block-instruct"><p>When you&#39;re ready to publish again, click "Publish Guide" to send it back to CityHow Editors for review.</p></div>';
			echo '<ul>';
			echo $btn_preview;
			echo '<li style="float:right;">';
			nh_frontend_delete_link($item_post_id);
		...
		elseif (isset($_GET['action'])) {
			...
			echo '<div class="alert alert-info"><a href="#" class="close" data-dismiss="alert">×</a><strong>This <a class="nhline" href="'.get_permalink($item_post_id).'" title="View your CityHow Guide" target="_blank">CityHow Guide</a> has been published!</strong></div>';
			}
//			echo '<div class="block-instruct"><p class="instructions"><strong>This <a href="'.get_permalink($item_post_id).'" title="View your CityHow Guide" target="_blank">CityHow Guide</a> has been published!</strong></p>';
			...
			echo '<p>When you&#39;re ready to publish again, click "Publish Guide" to send it back to CityHow Editors for review.</p></div>';
			echo '<ul>';
			echo $btn_preview;
			echo '<li style="float:right;">';
			nh_frontend_delete_link($item_post_id);
...
elseif ($current_user->ID != $mypost->post_author AND is_user_logged_in()) {
	...
	if ($_GET['ref'] == 'delete') {
		echo '<div class="alert alert-info"><a href="#" class="close" data-dismiss="alert">×</a><strong>Your Guide has been deleted.</strong></div>';
	}
	if ($gde_query->have_posts()) {
		echo '<div class="block-instruct"><p class="instructions">Looking for your CityHow Guides? Use the menu on the right to select an item to edit.</p></div>';	
	}
	else {
		echo '<div>You haven&#39;t created any CityHow Guides yet. <a class="nhline" href="'.$app_url.'/create-guide" title="Create a CityHow Guide">Get started</a> now, or <a class="nhline" href="'.$app_url.'/guides" title="Explore CityHow Guides">explore other Guides</a> for inspiration.</div>';
	}		
}
// NOT LOGGED IN
elseif (!is_user_logged_in()) {
	echo '<div class="block-instruct"><p class="instructions">Please <a class="nhline" href="'.$app_url.'/signin" title="Sign In to CityHow">sign in</a> to edit content.</p>';
	echo '<p style="margin-top:1.5em;"><a title="Sign In to CityHow" href="'.$app_url.'/signin" class="nh-btn-blue">Sign In</a>&nbsp;&nbsp;or&nbsp;&nbsp;<a title="Create an account" href="'.$app_url.'/register" class="nh-btn-blue">Create an Account</a></p></div>';
}
wp_reset_query();
?>
			</div><!--/ content-->	
	...
	if ($current_user->ID == $mypost->post_author) {
		...
		if ($count > 0) {
			...
			if ($guidequery->have_posts()) {
				echo '<h5 class="widget-title">CityHow Guides</h5>';
